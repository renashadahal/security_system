...
def draw_bounding_boxes(frame, detections, timestamp):
    """Draws rectangles and labels for detected motion."""
    for (box, label) in detections:
        ...
        cv2.rectangle(frame, (x, label_top), (x + text_w + 6, y), (0, 0, 255), -1)
        cv2.putText(frame, label, (x + 3, y - baseline - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
class VideoSaver:
    ...
    def start_saving(self, frame):
        """Initializes video writer for a new motion event."""
    ...
    def save_snapshot(self, frame):
        """Saves a static image of the trigger event."""
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        path = os.path.join(self.snapshot_folder, f"trigger_{ts}.jpg")
        cv2.imwrite(path, frame)

    def save_frame(self, frame):
        """Appends current frame to the video clip."""
